...
function _ForkedWorker(path, options) {

	if (!(this instanceof _ForkedWorker)) {
		return new _ForkedWorker(path, options);
	}

	var cmd = path;
	var args= [];
	if(process.env.running_under_istanbul && process.env.istanbul_config_file) {
		cmd = './node_modules/istanbul/lib/cli';
		args= ['--config='+process.env.istanbul_config_file, 'cover', '--report', 'none', '--print', 'none', '--include-pid', path];
	}

	var self = this;
	this._instance = fork(cmd, args, options || {})
